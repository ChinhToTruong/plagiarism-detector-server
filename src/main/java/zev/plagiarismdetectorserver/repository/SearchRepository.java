...
public class SearchRepository {
    ...
    public PageResponse<?> searchAdvance(int pageNo, int pageSize, String sortBy,String classroom, String[] search) {
    ...
    private List<User> getUsers(int pageNo, int pageSize, List<SearchCriteria> criteriaList,String classroom, String sortBy) {
        ...
        SearchCriteriaQueryConsumer consumer = new SearchCriteriaQueryConsumer(criteriaBuilder, predicate, root);

        // 2. xu ly join
        if (StringUtils.hasLength(classroom)) {
            Join<User, ClassRoom> classRoomJoin = root.join("classRooms", JoinType.INNER);
            Predicate classRoomPredicate = criteriaBuilder.like(classRoomJoin.get("name"), "%" + classroom + "%");
    ...
    public PageResponse<?> searchSpecification(Pageable pageable, String[] classroom, String[] user) {
        ...
        if (classroom != null && user != null){ // tim kiem co join
